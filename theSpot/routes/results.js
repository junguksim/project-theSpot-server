...
router.get('/', async (req, res) => { //x == longitude y == latitude
    ...
    if (req.query.category == undefined || req.query.station == undefined) { //partyIdx만 입력했을 경우 => 중간지점과 근처 역 5개 내놓기
        ...
        if (nearStationVerifyResult.length == 0) { //신규 검색일W 경우
            for (var i = 0; i < nearStationResult.stationList.length; i++) {
                console.log(nearStationResult.stationList[i].subwayXcnts);
                console.log('=============================================')
